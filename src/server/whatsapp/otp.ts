...
export async function rateLimitOTP(phone: string): Promise<true> {
    ...
    if (recentToken?.metadata && typeof recentToken.metadata === "object" && "phone" in recentToken.metadata) {
...
export async function sendPhoneOtp(phone: string): Promise<{ verificationId: string }> {
    ...
    const [token] = await db.insert(UserTokensTable).values({
        userId: null,
        tokenHash,
        type: "otp",
...
export async function verifyPhoneOtp(phone: string, otp: string): Promise<{ verificationId: string }> {
    const tokenHash = hashTokenValue(otp);
    ...
    if (!token || !token.metadata || typeof token.metadata !== "object" || !("phone" in token.metadata)) {
...
export async function assertPhoneVerified(phone: string, verificationId: string): Promise<void> {
    const token = await db.query.UserTokensTable.findFirst({
        where: and(
    ...
    if (!token?.consumedAt || !token.metadata || typeof token.metadata !== "object") {
